...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfileImage = async () => {
      try {
        const response = await fetch('http://192.168.0.27:8000/user/getimg', {
          headers: {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (!file || !file.type.startsWith('image/*')) {
      setMessage({ type: 'error', content: 'Bármiyen formátumú képek tölthetők fel!' });
      return;
    }

    //if (file.size > 2 * 1024 * 1024) {
    //    setMessage({ type: 'error', conetnt: 'A kép mérete nem haladhatja meg a 2MB-ot!' });
    //    return;
    //}
